...
COLOR_HFOV_DEG = 60.0   # Astra RGB 카메라 수평 시야각 근사값
...
def calc_angle_x_deg(cx, image_width):
    ...
    angle_x = norm_x * (COLOR_HFOV_DEG / 2.0)
    return float(angle_x)
    ...
    try:
        while True:
            ...
            for box in results[0].boxes:
                ...
                cx = int((x1 + x2) / 2)
                cy = int((y1 + y2) / 2)
                angle_x_deg = calc_angle_x_deg(cx, cw)
            ...
            cv2.putText(
                bgr,
                f"{depth_text} | angle: {angle_x_deg:.1f} deg",
                (x1, max(20, y1 - 8)),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.5,
                (0, 255, 255),
